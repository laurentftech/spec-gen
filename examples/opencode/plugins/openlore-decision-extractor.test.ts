...
import { scoreFromDepGraph } from './lib/openlore-decision-extractor-helpers.ts';
...
describe('scoreFromDepGraph', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'openlore-extractor-test-'));
  ...
  it('returns null when dependency-graph.json is invalid JSON', async () => {
    await writeFile(join(tmpDir, '.openlore', 'analysis', 'dependency-graph.json'), 'not json');
    const result = scoreFromDepGraph('src/foo.ts', tmpDir);
    expect(result).toBeNull();
  });
  ...
  it('returns null when file is not in the graph', async () => {
    await writeFile(
      join(tmpDir, '.openlore', 'analysis', 'dependency-graph.json'),
