...
function presentDecisions(decisions: any[]): void {
  ...
  decisions.forEach((d, i) => {
    ...
    if (d.affectedFiles?.length)
      console.log(`   Files      : ${d.affectedFiles.join(", ")}`)
  })
  console.log(`\n${sep}`)
...
export const SpecGenEnforcer: Plugin = async () => {
  let toolCallCount = 0
  ...
  return {
    ...
    "tool.execute.before": async (input: any, output: any) => {
      ...
      const covered = getActiveDecisions().some(d =>
        (d.affectedFiles ?? []).includes(filePath),
      )
      if (!covered) {
    ...
    event: async ({ event }: any) => {
      ...
      if (gate.verified?.length) {
        console.log(
          `⛔ ${gate.verified.length} decision(s) await human approval before commit:\n`,
        )
